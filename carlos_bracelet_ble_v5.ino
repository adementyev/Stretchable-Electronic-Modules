...
C:\Users\artemPC\AppData\Local\Arduino15\packages\adafruit\hardware\nrf52\0.13.0\libraries\Wire
...
boolean READ_ORIENTATION = true;
boolean READ_PPG = true;
boolean READ_TEMPERATURE = true;
...
//  Serial.println("---------------------------\n");
  ...
  Bluefruit.setName("groki_master");
...
void loop()
{
    ...
    char buf[64];
    counter_temperature=counter_temperature+1;
    counter_imu = counter_imu + 1;
    
    if(READ_TEMPERATURE && counter_temperature ==100) { 
      float temperature_reading_si7051 = si7051.readTemperature(); 
      
      char temperature_string_si7051[5];
      
      snprintf(temperature_string_si7051, 6, "%f", temperature_reading_si7051);
      snprintf(buf, sizeof buf, "T,%s\r\n", temperature_string_si7051 );

      bleuart.write( buf, 10 );
      //delay(50);
   ...
   // delay(2);
    ...
    if(READ_ORIENTATION && counter_imu == 5) { 
          counter_imu = 0 ;
